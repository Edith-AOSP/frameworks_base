...
class EdithClockFrauncesViewGroup(clockCtx: EdithClockFrauncesContext) :
    DigitalClockViewGroup<EdithClockFrauncesTextView>(clockCtx) {

    override val children: Sequence<EdithClockFrauncesTextView>
        get() = (this as ViewGroup).children.filterIsInstance<EdithClockFrauncesTextView>()
    ...
    override fun getChildFrame(child: EdithClockFrauncesTextView): VRectF {
        val yBuffer = context.resources.getDimensionPixelSize(R.dimen.clock_vertical_digit_buffer)
        var offset =
            maxChildSize.run {
                when (child.id) {
