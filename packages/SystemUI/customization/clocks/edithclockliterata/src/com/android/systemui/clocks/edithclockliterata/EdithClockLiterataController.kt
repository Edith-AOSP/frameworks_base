...
class EdithClockLiterataController(
    ...
    override val config =
        ClockConfig(
            EdithClockLiterataProvider.CLOCK_ID,
            clockCtx.resources.getString(R.string.edith_literata_clock_name),
            clockCtx.resources.getString(R.string.edith_literata_clock_description),
        )
    ...
    override fun initialize(isDarkTheme: Boolean, dozeFraction: Float, foldFraction: Float) {
        smallClock.run {
            ...
            maxSizeListener = { eventListeners.fire { onMaxSizeChanged(it, isLargeClock = false) } }
        ...
        largeClock.run {
            ...
            maxSizeListener = { eventListeners.fire { onMaxSizeChanged(it, isLargeClock = true) } }
            events.onThemeChanged(theme.copy(isDarkTheme = isDarkTheme))
            animations.onFontAxesChanged(clockCtx.settings.axes)
            animations.doze(dozeFraction)
            animations.fold(foldFraction)
